...
class App extends Component {
	...
	_clearPreviousResps() {
		this.setState(state => {
			const resps = state.resps.map(rr => { rr.opacity = 0.2; return rr })
			if (resps.length) {
				resps[0].clear = 'left'
			}
			return {
	...
	_pushError(err) {
		this.setState(state => ({ resps: [...state.resps, { error: err.message, meta: err.meta, code: err.code, key: ++this.respCounter }] }))
	}
		...
		{resps.map(resp => (
			<pre
				...
				style={{
					fontSize: '9pt',
					float: 'left',
					backgroundColor: resp.error ? '#c41' : '#444',
					color: 'white',
					padding: '5px',
					margin: '5px',
					textAlign: 'left',
