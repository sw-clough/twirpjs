...
/*
Package twirper is a generated twirp stub package.
...
func (c *twirperProtobufClient) Echo(ctx context.Context, in *EchoReq) (*EchoReq, error) {
...
func (c *twirperJSONClient) Echo(ctx context.Context, in *EchoReq) (*EchoReq, error) {
	ctx = ctxsetters.WithPackageName(ctx, "twirper")
	ctx = ctxsetters.WithServiceName(ctx, "Twirper")
	ctx = ctxsetters.WithMethodName(ctx, "Echo")
	out := new(EchoReq)
...
func (s *twirperServer) writeError(ctx context.Context, resp http.ResponseWriter, err error) {
	writeError(ctx, resp, err, s.hooks)
}

// TwirperPathPrefix is used for all URL paths on a twirp Twirper server.
// Requests are always: POST TwirperPathPrefix/method
// It can be used in an HTTP mux to route twirp requests along with non-twirp requests on other routes.
const TwirperPathPrefix = "/twirp/twirper.Twirper/"

func (s *twirperServer) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	ctx := req.Context()
	ctx = ctxsetters.WithPackageName(ctx, "twirper")
	ctx = ctxsetters.WithServiceName(ctx, "Twirper")
...
func (s *twirperServer) serveEcho(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
...
func (s *twirperServer) serveEchoJSON(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	var err error
	ctx = ctxsetters.WithMethodName(ctx, "Echo")
...
func (s *twirperServer) serveRepeatProtobuf(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	...
	// Call service method
	var respStream RepeatRespStream
	func() {
		defer func() {
			// In case of a panic, serve a 500 error and then panic.
			if r := recover(); r != nil {
...
func (r protoStreamReader) Read(msg proto.Message) error {
	...
	// This is a trailer, read it
	defer r.c.Close()
	buf := make([]byte, int(l))
	_, err = io.ReadFull(r.r, buf)
	if err != nil {
		return wrapErr(err, "failed to read trailer")
	}
	...
	if err != nil {
		return wrapErr(err, "failed to decode trailer")
	}
	if trailer == io.EOF.Error() || trailer == "OK" {
		return io.EOF
	}
	if trailer == "" {
		return twirp.InternalError("empty trailer")
	}
	return twirp.InternalError(trailer)
}
